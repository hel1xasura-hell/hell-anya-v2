...
class JSONDatabase(Database):
    ...
    async def connect(self) -> None:
        async with self._lock:
            ...
            # Backward-compatible migration: older database files predate the
            # audit_log / known_users buckets used by the web dashboard.
            migrated = False
            for key, default_value in _DEFAULT_STATE.items():
                if key not in self._state:
                    self._state[key] = json.loads(json.dumps(default_value))
                    migrated = True
            if migrated:
                self._flush_unlocked()
    ...
    async def log_command(
        ...
        async with self._lock:
            ...
            log = self._state["audit_log"]
            log.append(entry)
            if len(log) > _AUDIT_LOG_MAX_ENTRIES:
                del log[: len(log) - _AUDIT_LOG_MAX_ENTRIES]
            self._flush_unlocked()
    ...
    async def upsert_user(self, user_id: int, username: str, full_name: str) -> None:
        async with self._lock:
            bucket = self._state["known_users"]
            key = str(user_id)
            now = time.time()
            ...
            if existing is None:
                ...
                existing["command_count"] = existing.get("command_count", 0) + 1
            self._flush_unlocked()

    async def get_users(self) -> list[dict[str, Any]]:
        async with self._lock:
            users = list(self._state["known_users"].values())
